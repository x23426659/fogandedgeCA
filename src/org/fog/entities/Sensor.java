...
public class Sensor extends SimEntity {
	...
	// Fixed: Set sensorName to "Sensor" by default so it matches AppEdge source module
	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation, 
		...
		setSensorName("Sensor");  // FIX: set sensorName to "Sensor" (matches app edge source)
		setLatency(latency);
	}
	...
	public Sensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation, 
		...
		setUserId(userId);
		setTupleType(tupleType);
		setSensorName("Sensor");  // FIX: same here
		setLatency(latency);
	}
	...
	public void transmit(){
		...
		for(AppEdge edge : getApp().getEdges()){
			if(edge.getSource().equals(getSensorName()) && edge.getTupleType().equals(getTupleType()))
				_edge = edge;
		}
		if(_edge == null){
		    Logger.error(getName(), "No matching AppEdge found for sensor with source module " + getSensorName() + " and tuple type " + getTupleType());
