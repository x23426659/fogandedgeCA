...
public class Controller extends SimEntity{
	...
	private void processTupleFinished(SimEvent ev) {
		// You can implement tuple finished processing if needed
	}
	...
	public Map<String, Double> getApplicationLoopDelays(String appId) {
	    Map<String, Double> loopDelays = new HashMap<>();
	    Application app = applications.get(appId);
	    if (app == null) return loopDelays;

	    for (AppLoop loop : app.getLoops()) {
	        Double delay = TimeKeeper.getInstance().getLoopIdToCurrentAverage().get(loop.getLoopId());
	        if (delay != null) {
	            String loopName = loop.getModules().toString();
	            loopDelays.put(loopName, delay);
	        }
	    }
	    return loopDelays;
	}
